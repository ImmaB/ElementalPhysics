...
struct b2SeparationFunction
{
		...
		float32 t1)
	{
		...
		if (count == 1)
		{
			...
			Vec2 pointA = b2Mul(xfA, localPointA);
			Vec2 pointB = b2Mul(xfB, localPointB);
			m_axis = pointB - pointA;
			float32 s = m_axis.Normalize();
			m_localPoint = Vec2_zero;
			return s;
		}
		else if (cache.indexA[0] == cache.indexA[1])
		{
			...
			m_axis.Normalize();
			Vec2 normal = b2Mul(xfB.q, m_axis);

			m_localPoint = 0.5f * (localPointB1 + localPointB2);
			Vec2 pointB = b2Mul(xfB, m_localPoint);
		...
		else
		{
			...
			m_axis.Normalize();
			Vec2 normal = b2Mul(xfA.q, m_axis);

			m_localPoint = 0.5f * (localPointA1 + localPointA2);
			Vec2 pointA = b2Mul(xfA, m_localPoint);
	...
	float32 FindMinSeparation(int32& indexA, int32& indexB, float32 t) const
	{
		...
		case e_points:
			{
				Vec2 axisA = b2MulT(xfA.q,  m_axis);
				Vec2 axisB = b2MulT(xfB.q, -m_axis);
		...
		case e_faceA:
			{
				...
				Vec2 axisB = b2MulT(xfB.q, -normal);
		...
		case e_faceB:
			{
				...
				Vec2 axisA = b2MulT(xfA.q, -normal);
