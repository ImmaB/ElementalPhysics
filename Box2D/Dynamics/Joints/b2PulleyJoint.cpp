				...
				float32 r)
{
	...
	localAnchorA = bA.GetLocalPoint(anchorA);
	localAnchorB = bB.GetLocalPoint(anchorB);
...
b2PulleyJoint::b2PulleyJoint(const b2PulleyJointDef* def, b2World& world)
: b2Joint(def, world)
{
...
void b2PulleyJoint::SolveVelocityConstraints(const b2SolverData& data)
{
	...
	Vec2 vpA = vA + b2Cross(wA, m_rA);
	Vec2 vpB = vB + b2Cross(wB, m_rB);
...
Vec2 b2PulleyJoint::GetReactionForce(float32 inv_dt) const
{
...
{
	Vec2 p = GetBodyA().GetWorldPoint(m_localAnchorA);
...
{
	Vec2 p = GetBodyB().GetWorldPoint(m_localAnchorB);
