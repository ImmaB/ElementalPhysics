...
b2ContactSolver::b2ContactSolver(const b2ContactSolverDef& def, b2World& world) : m_world(world)
{
	m_step = def.step;
	m_allocator = def.allocator;
	m_count = def.count;
	m_positionConstraints.resize(m_count);
	m_velocityConstraints.resize(m_count);
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		const float32 radiusA = m_world.GetShape(fixtureA).m_radius;
		const float32 radiusB = m_world.GetShape(fixtureB).m_radius;
		...
		vc.invMassA = bodyA.IsType(Body::Type::Dynamic) ? bodyA.m_invMass : 0.0f;
		vc.invMassB = bodyB.IsType(Body::Type::Dynamic) ? bodyB.m_invMass : 0.0f;
...
template <typename F>
void b2ContactSolver::ForEachContactVelConstraint(const F& func)
{
	for (int32 i = 0; i < m_count; ++i)
		func(m_velocityConstraints[i]);
}

template <typename F>
void b2ContactSolver::ForEachContactPosConstraint(const F& func)
{
	for (int32 i = 0; i < m_count; ++i)
		func(m_positionConstraints[i]);
}
template <typename F>
void b2ContactSolver::ForEachContactVelAndPosConstraint(const F& func)
{
	for (int32 i = 0; i < m_count; ++i)
		func(m_velocityConstraints[i], m_positionConstraints[i]);
}

// Initialize position dependent portions of the velocity constraints.
void b2ContactSolver::InitializeVelocityConstraints()
{
	ForEachContactVelAndPosConstraint(
		[&](b2ContactVelocityConstraint& vc, b2ContactPositionConstraint& pc)
	{
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			Vec2 tangent = b2Cross(vc.normal, 1.0f);
...
{
	// Warm start.
	ForEachContactVelConstraint([&](const b2ContactVelocityConstraint& vc)
	{
		int32 indexA = vc.indexA;
		int32 indexB = vc.indexB;
		float32 mA = vc.invMassA;
		...
		Vec2 tangent = b2Cross(normal, 1.0f);
...
{
	ForEachContactVelConstraint([&](b2ContactVelocityConstraint& vc)
	{
		int32 indexA = vc.indexA;
		int32 indexB = vc.indexB;
		float32 mA = vc.invMassA;
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			Vec2 dv = vB + b2Cross(wB, vcp.rB) - vA - b2Cross(wA, vcp.rA);
		...
		else
		{
			...
			// Relative velocity at contact
			Vec2 dv1 = vB + b2Cross(wB, cp1.rB) - vA - b2Cross(wA, cp1.rA);
			Vec2 dv2 = vB + b2Cross(wB, cp2.rB) - vA - b2Cross(wA, cp2.rA);
...
{
	void Initialize(const b2ContactPositionConstraint& pc, const b2Transform& xfA, const b2Transform& xfB, int32 index)
	{
		...
		case b2Manifold::e_circles:
			{
				Vec2 pointA = b2Mul(xfA, pc.localPoint);
				Vec2 pointB = b2Mul(xfB, pc.localPoints[0]);
		...
		case b2Manifold::e_faceA:
			{
				...
				Vec2 planePoint = b2Mul(xfA, pc.localPoint);

				Vec2 clipPoint = b2Mul(xfB, pc.localPoints[index]);
		...
		case b2Manifold::e_faceB:
			{
				...
				Vec2 planePoint = b2Mul(xfB, pc.localPoint);

				Vec2 clipPoint = b2Mul(xfA, pc.localPoints[index]);
				separation = b2Dot(clipPoint - planePoint, normal) - pc.radiusA - pc.radiusB;
				point = clipPoint;
...
bool b2ContactSolver::SolvePositionConstraints()
{
	...
	ForEachContactPosConstraint([&](const b2ContactPositionConstraint& pc)
	{
